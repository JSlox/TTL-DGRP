...
# receive a data.frame with column "KO" with KO times, "sex" with sex, "temp" with temperature, "Geno" with Geno line.
TDT <- function(matriz){
  #### CTmax and Z estimation ####
  
  # Calculate the knockdown time mean for each Genotype and sex
  rep.st37 <- round(with(subset(matriz,temp=="37"),tapply(KO,list(Geno,sex),mean)),2)
  rep.st38 <- round(with(subset(matriz,temp=="38"),tapply(KO,list(Geno,sex),mean)),2)
  rep.st39 <- round(with(subset(matriz,temp=="39"),tapply(KO,list(Geno,sex),mean)),2)
  rep.st40 <- round(with(subset(matriz,temp=="40"),tapply(KO,list(Geno,sex),mean)),2)
  
  mean.rep <- with(matriz,tapply(KO,list(temp,Geno,sex),mean))		# Calculate the knockdown time mean for each temperature, Geno line and sex
  nlines <- length(unique(matriz$Geno)) # extract number of genotypes from data set
  ...
  Geno <- c(levels(matriz$Geno))	# Take Geno levels and..
  Geno <- factor(rep(Geno,2))	# Create a factor with them
  tdt <- data.frame(Geno,sex,out)
  return(tdt)
}